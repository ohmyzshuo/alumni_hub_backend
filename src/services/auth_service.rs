...
use crate::utils::auth_utils::{check_password_hash, extract_login_name, AppError};
...
impl AuthService {
    ...
    pub async fn authenticate_user(
        ...
        let mut conn = self.pool.get().map_err(|e| AppError { message: e.to_string() })?;

        if role == "alumni" {
            let login_name = extract_login_name(username)?;

            let alumnus: Alumni = alumnis::dsl::alumnis
                .filter(alumnis::dsl::matric_no.ilike(format!("{}%", login_name)))
                .first::<Alumni>(&mut conn)
            ...
            let staff: Staff = staffs::dsl::staffs
